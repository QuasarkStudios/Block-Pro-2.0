...
class LogInViewController2: UIViewController, UITextFieldDelegate, BEMCheckBoxDelegate {
    ...
    let splashImageView = UIImageView(image: UIImage(named: "Launch-Log In Screen Illustration (Scaled)"))
    
    var splashImageViewTopAnchor: NSLayoutConstraint?
    var splashImageViewWidthConstraint: NSLayoutConstraint?
    var splashImageViewHeightConstraint: NSLayoutConstraint?
    ...
    private func configureSplashScreen () {
        ...
        splashImageViewTopAnchor = splashImageView.topAnchor.constraint(equalTo: self.view.topAnchor, constant: (UIScreen.main.bounds.height / 2) - 200)
        splashImageViewTopAnchor?.isActive = true
        
        splashImageView.centerXAnchor.constraint(equalTo: self.view.centerXAnchor, constant: 0).isActive = true
        
        splashImageViewWidthConstraint = splashImageView.widthAnchor.constraint(equalToConstant: UIScreen.main.bounds.width - 100)
        splashImageViewWidthConstraint?.isActive = true
        
        splashImageViewHeightConstraint = splashImageView.heightAnchor.constraint(equalToConstant: 400)
        splashImageViewHeightConstraint?.isActive = true
    ...
    private func animateSplashScreen () {
        ...
        splashImageViewTopAnchor?.constant = 75
        splashImageViewWidthConstraint?.constant = 250
        splashImageViewHeightConstraint?.constant = 250
        
        UIView.animate(withDuration: 1, delay: 0, usingSpringWithDamping: 0.75, initialSpringVelocity: 0, options: .curveEaseInOut) {
