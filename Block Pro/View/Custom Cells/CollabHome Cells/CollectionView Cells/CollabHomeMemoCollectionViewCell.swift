//
//  CollabHomeMemoCollectionViewCell.swift
//  Block Pro
//
...
class CollabHomeMemoCollectionViewCell: UICollectionViewCell {
    ...
    private func retrieveVoiceMemo (_ collabID: String, _ voiceMemoID: String, _ completion: @escaping ((_ error: Error?) -> Void)) {
    ...
    //Called from the CollabHomeVoiceMemoCell
    func cellTapped () {
        ...
        if recordingPlaying ?? false {
            ...
            if let collabID = collab?.collabID, let voiceMemoID = voiceMemo?.voiceMemoID {
                ...
                if FileManager.default.fileExists(atPath: documentsDirectory.path + "/VoiceMemos" + "/\(voiceMemoID).m4a") {
                ...
                //If the voiceMemo hasn't been loaded
                    ...
                    retrieveVoiceMemo(collabID, voiceMemoID) { [weak self] (error) in
