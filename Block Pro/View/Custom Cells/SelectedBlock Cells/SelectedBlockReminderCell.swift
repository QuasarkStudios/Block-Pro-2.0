        ...
        didSet {
            ...
            notificationScheduler.getPendingNotifications { [weak self] (requests) in
                ...
                DispatchQueue.main.async {
                    ...
                    self?.configureReminderLabel(notificationRequests: requests)
                }
            }
        }
    }
...
//        configureReminderLabel()
    ...
    private func configureReminderLabel (notificationRequests: [UNNotificationRequest]) {
        ...
        if formatter != nil {
            ...
            let semiBoldText: [NSAttributedString.Key : Any] = [NSAttributedString.Key.font : UIFont(name: "Poppins-SemiBold", size: reminders.count > 0 ? 15 : 17) as Any, NSAttributedString.Key.foregroundColor : UIColor.black, NSAttributedString.Key.paragraphStyle : paragraphStyle]
            ...
            else if reminders.count == 2 {

                attributedString.append(NSAttributedString(string: "Remind me at: ", attributes: semiBoldText))

                reminders = reminders.sorted() //Sorting the reminders

                if let time = block?.starts, let firstReminder = calendar.date(byAdding: .minute, value: minutesToSubtractBy[reminders[1]], to: time), let secondReminder = calendar.date(byAdding: .minute, value: minutesToSubtractBy[reminders[0]], to: time) {
