//
//  VoiceMemosConfigurationCell.swift
//  Block Pro
//
...
class VoiceMemosConfigurationCell: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        super.init(style: .default, reuseIdentifier: "voiceMemosConfigurationCell")
    ...
    private func configureCollectionView () {
        ...
        memoCollectionView.register(VoiceMemoConfigurationCollectionViewCell.self, forCellWithReuseIdentifier: "voiceMemoConfigurationCollectionViewCell")
    }
    ...
    func presentDeniedAlert () {
        ...
        else if let viewController = voiceMemosConfigurationDelegate as? ConfigureBlockViewController {
    ...
    @objc func endRecording () {
        ...
        voiceMemoRecorder?.stopRecording(voiceMemoID: idForVoiceMemoBeingRecorded, completion: { (audioDurationSeconds) in
            ...
            voiceMemosConfigurationDelegate?.voiceMemoSaved(newVoiceMemo)
        })
    ...
    func deleteVoiceMemo (_ voiceMemo: VoiceMemo) {
        ...
        voiceMemosConfigurationDelegate?.voiceMemoDeleted(voiceMemo)
    }
        ...
        if !keyboardPresent {
            ...
            if nameTextFieldSelected {
                ...
                if let viewController = voiceMemosConfigurationDelegate as? ConfigureCollabViewController {
                    ...
                    //y-coord for the voiceMemoConfigurationCell in the ConfigureBlockViewController
                    let createMemoCellMinY = tableView.rectForRow(at: IndexPath(row: 5, section: 0)).minY
                    
                    //Distance from the top of the voiceMemoConfigurationCell to the top of the VoiceMemo collection view cell, i.e. 40; + the itemSize; - (half the height of the nameTextField + the bottomAnchor of the nameTextField), i.e. 10.5
                    let nameTextFieldCenter = 40 + floor(itemSize) - 10.5
                    
                    //y-coord of the configureBlockTableView in regards to the keyWindow
                    let tableViewMinY = viewController.view.convert(tableView.frame, to: keyWindow).minY
    ...
    @objc func attachButtonPressed () {
        ...
        if voiceMemoRecorder?.microphoneAccessGranted ?? false {
            ...
            voiceMemosConfigurationDelegate?.attachMemoSelected()
    ...
    @objc private func recordingCancelled () {
        ...
        voiceMemosConfigurationDelegate?.recordingCancelled()
    }
...
extension VoiceMemosConfigurationCell: UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "voiceMemoConfigurationCollectionViewCell", for: indexPath) as! VoiceMemoConfigurationCollectionViewCell
...
extension VoiceMemosConfigurationCell: UITextFieldDelegate {
    
    func textFieldDidEndEditing(_ textField: UITextField) {
        
        for cell in memoCollectionView.visibleCells {
            
            if let voiceMemoCell = cell as? VoiceMemoConfigurationCollectionViewCell {
                
                if let voiceMemoIndex = voiceMemos?.firstIndex(where: { $0.voiceMemoID == voiceMemoCell.voiceMemo?.voiceMemoID }) {
                    
                    //Saving the name of the voiceMemos, most importantly the one that has just finished being edited
                    voiceMemos?[voiceMemoIndex].name = voiceMemoCell.nameTextField.text
                }
            }
        }
    }
